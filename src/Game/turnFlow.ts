  ...
  switch (game.stateKind) {
    ...
    case 'doubled': {
      ...
      const [activePlayerForColor, inactivePlayerForColor] =
        getPlayersForColor(players, activeColor!)
 ...
 */
export function moveAndFinalize(
  game: BackgammonGameMoving,
  checkerId: string
): BackgammonGameMoving | BackgammonGameMoved | BackgammonGameCompleted {
  const moved = move(game, checkerId)
  if (moved.stateKind === 'moving') {
    // Let CORE decide if the turn should complete now
    return checkAndCompleteTurn(moved as BackgammonGameMoving) as
      | BackgammonGameMoving
      | BackgammonGameMoved
      | BackgammonGameCompleted
  }
 ...
 */
export function executeAndRecalculate(
  game: BackgammonGameMoving,
  originId: string,
  options?: MoveExecutionOptions
  ...
  const gameAfterMove = move(game, checkerInOrigin.id, undefined, options)
  ...
  const gameAfterTurnCheck = checkAndCompleteTurn(movingGame)
  ...
  if (
    ...
    return confirmTurn(gameAfterTurnCheck as BackgammonGameMoved)
  }
 ...
 */
export function checkAndCompleteTurn(
  game: BackgammonGameMoving
): BackgammonGame {
...
export function getPlayersForColor(
